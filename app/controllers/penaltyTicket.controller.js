...
export const findAll = async (req, res, next) => {
    ...
    const penaltyTickets = await penaltyTicketservice.findPagination({ page: page, limit: limit, paymentStatus: paymentStatus });
    return res.status(200).json(
...
export const deletePenaltyTicket = async (req, res, next) => {
    try {
        ...
        return next(
            new ApiError(500, `Could not delete Penalty ticket with id=${req.params.id}`)
        );
    }
}